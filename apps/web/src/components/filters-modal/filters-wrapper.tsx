...
const FiltersWrapper = React.forwardRef<HTMLDivElement, FiltersWrapperProps>(
  ({ items, title = 'Filters', showTitle = true, showActions = true, className, scrollShadowClassName }, ref) => {
    const renderFilter = React.useCallback((filter: Filter) => {
      switch (filter.type) {
        ...
        case FilterTypeEnum.CheckboxGroup:
          return (
            <Accordion className="px-0" defaultExpandedKeys={filter?.defaultOpen ? ['options'] : []}>
              <AccordionItem
                key="options"
                classNames={{
        ...
        case FilterTypeEnum.Color:
          return (
            ...
            >
              {filter.options?.map((option) => (
                <ColorRadioItem key={option.value} color={option.color} tooltip={option.title} value={option.value} />
              ))}
            </RadioGroup>
          );
      }
    ...
    return (
      <div ref={ref} className={cn('h-full max-h-fit overscroll-y-auto w-full max-w-sm rounded-medium bg-content1 p-4', className)}>
        <ScrollShadow
          className={cn(
            '-mx-6 h-full px-6',
            {
            ...
            {items.map((filter) => (
              <div key={filter.title} className="flex flex-col gap-3">
                {filter.type !== FilterTypeEnum.CheckboxGroup ? (
                  <div>
                    <h3 className="text-medium font-medium leading-8 text-default-600">{filter.title}</h3>
        ...
        {showActions && (
          <>
            ...
            <div className="mt-auto flex flex-col gap-2">
              <Button
                color="primary"
                startContent={<Icon className="text-primary-foreground [&>g]:stroke-[3px]" icon="solar:magnifer-linear" width={16} />}
              >
                Show 300+ stays
              </Button>
