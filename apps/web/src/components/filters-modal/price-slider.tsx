...
const PriceSlider = React.forwardRef<HTMLDivElement, PriceSliderProps>(({ range, animation, className, ...props }, ref) => {
  ...
  const rangePips = React.useMemo(() => {
    return Array.from({ length: 32 }).map((_, index) => {
      ...
      const isInRange = pipValuePercentage >= rangePercentageValue[0] + 5 && pipValuePercentage <= rangePercentageValue[1] + 2;
